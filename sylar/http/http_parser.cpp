...
static auto g_http_request_buffer_size = sylar::Config::Lookup("http.request.buffer_size", 
    (uint64_t)4 * 1024, "http request buffer size");
static auto g_http_request_max_body_size = sylar::Config::Lookup("http.request.max_bopdy_size", 
    (uint64_t)64 * 1024, "http request max body size");
static auto g_http_response_buffer_size = sylar::Config::Lookup("http.response.buffer_size", 
    (uint64_t)4 * 1024, "http response buffer size");
static auto g_http_response_max_body_size = sylar::Config::Lookup("http.response.max_bopdy_size", 
...
void on_request_method(void *data, const char *at, size_t length)  {
    ...
    HttpMethod m = CharsToHttpMethod(std::string_view(at, length));
    if (m == HttpMethod::INVALID_METHOD) {
        SYLAR_LOG_WARN(g_logger) << "invalid http request method: " << std::string_view(at, length);
...
HttpRequestParser::HttpRequestParser() : m_data(new HttpRequest()), m_error(0) {
    http_parser_init(&m_parser);
    m_parser.request_method = on_request_method;
    m_parser.request_uri = on_request_uri;
    m_parser.fragment = on_requese_fragment;
...
HttpResponseParser::HttpResponseParser() : m_data(new HttpResponse()), m_error(0) {
