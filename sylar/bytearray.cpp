...
namespace sylar {
    ...
    void ByteArray::write(const void* buf, size_t size) {
        ...
        size_t ncap = m_cur->size - npos;                       // 当前节点容量   m_cur->size == m_baseSize ???
    ...
    bool ByteArray::writeToFile(std::string_view name) {
        ...
        if (!ofs) {
            SYLAR_LOG_ERROR(g_logger) << "writeToFile name = " << name << " error , errno = " 
    ...
    bool ByteArray::readFromFile(std::string_view name) {
        ...
        if (!ifs) {
            SYLAR_LOG_ERROR(g_logger) << "readFromFile name = " << name << " error, errno = " 
             << errno << " errstr = " << strerror(errno);
            return false;
        }
        ptr<char> buff(new char[m_baseSize], [](char* ptr) { delete[] ptr; });
        while (!ifs.eof()) {
